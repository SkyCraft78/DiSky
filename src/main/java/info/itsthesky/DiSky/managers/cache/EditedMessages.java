...
public class EditedMessages extends ListenerAdapter {
    ...
    public void onReady(ReadyEvent e) {
        for (Guild guild : e.getJDA().getGuilds()) {
            Utils.async(() -> {
                if (Utils.INFO_CACHE) DiSky.getInstance().getLogger().info("Starting cache of event "on message edit" for guild " + guild.getName() + "...");
                ...
                for (TextChannel channel : guild.getTextChannels()) {
                    try {
                        ...
                        if (Utils.INFO_CACHE) DiSky.getInstance().getLogger().warning("DiSky cannot cache messages for "on message edit" event, since the bot doesn't have the " + ex.getPermission().getName() + " permission!");
                    }
                }
                if (Utils.INFO_CACHE) DiSky.getInstance().getLogger().info("Finished caching messages for "on message edit" event for guild " + guild.getName() + ". Took " + (start - System.currentTimeMillis()) + "ms!");
            });
        }
    }
